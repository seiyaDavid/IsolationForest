...
def sample_data():
    ...
    Returns:
        pd.DataFrame: DataFrame containing:
            - Date column
            - STOCK_1 with one anomaly
            - STOCK_2 with no anomalies
    """
    dates = pd.date_range(start="2023-01-01", periods=100, freq="D")
    df = pd.DataFrame({"Date": dates})
    df["STOCK_1"] = [0.01] * 99 + [0.5]  # Last value is anomalous
    df["STOCK_2"] = [0.01] * 100  # No anomalies
    return df
...
def test_detect_anomalies_endpoint(sample_data):
    ...
    assert "STOCK_1" in data["anomalous_stocks"]
    assert "STOCK_2" not in data["anomalous_stocks"]
